...
remotes::install_github("Duzhaozhen-BioC/OmniAgeR")
```
...
```{r run-mototic-clock}
epiMarkerRes <- suppressMessages(epiMarker(
    x = lungInvSeObj,
    clockNames = "mitotic",
...
epiMarkerOut <- suppressMessages(epiMarker(hannumBmiqM,
...
data("ScPbmcExample")
set.seed(42)
scImmuAgingOut <- suppressMessages(scImmuAging(
   x = ScPbmcExample,
   cellTypes = c("CD4T"),
   assayName = "logcounts",
   donorCol = "donor_id",
...
```{r brain-ct-clock}
suppressMessages(library(OmniAgeR))
library(glmnet)
library(ggplot2)
library(patchwork)
library(ggpubr)
...
clockResults <- suppressMessages(brainCtClock(
    x = ScBrainExample,

...
#' compcrpOut <- compCRP(hannum_bmiq_m)



compCRP <- function(betaM, minCoverage = 0.5, verbose = TRUE) {
  
  data("crpCpG", envir = environment())
  scoreList <- list()
  sigNames <- names(crpCpG.lv)
  
  for (i in seq_along(crpCpG.lv)) {
    currentSigName <- sigNames[i]
    sigWeights <- crpCpG.lv[[i]]
    ...
    if (length(keepIdx) < length(rowSdsV)) {
      if (verbose) {
        message(sprintf("[%s] Excluding %d constant probes.", 

...
#'
#' dnamFitAgeOut <- dnamFitAge(hannum_bmiq_m, age, sex, GrimAge1.o$DNAmGrimAge1)
...
dnamFitAge <- function(betaM, age, sex, grimageVector, minCoverage = 0.5, 
  ...
  data("DNAmFitAgeCoef", envir = environment())
...
.estimateFitnessMarkers <- function(data, modelData) {
  ...
  for (marker in names(clocks)) {
    ...
    scores[data$Female == 1] <- .applyTidyModel(data[data$Female == 1, ], 
                                                femModel)
    scores[data$Female == 0] <- .applyTidyModel(data[data$Female == 0, ], 
                                                maleModel)
    res[[marker]] <- scores
  }
...
.calculateFinalFitAge <- function(data) {
  ...
  compIdx <- stats::complete.cases(data[, c("DNAmGait_noAge", "DNAmGrip_noAge", 

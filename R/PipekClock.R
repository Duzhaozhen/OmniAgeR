...
#' pipekElasticNetOut <- pipekElasticNet(hannum_bmiq_m)
#'

pipekElasticNet <- function(betaM,
                             minCoverage = 0.5, 
                             verbose = TRUE) {
  data("PipekElasticNet_Coef", envir = environment())
  
  predAgev <- .calLinearClock(betaM, PipekElasticNet_Coef, "pipekElasticNet", 
                              minCoverage,verbose)
  # transformation
...
#' downloadOmniAgeRExample("Hannum_example")
#' loadOmniAgeRExample("Hannum_example")
#' pipekFilteredhOut <- pipekFilteredh(hannum_bmiq_m)
#'

pipekFilteredh <- function(betaM,
                            minCoverage = 0.5, 
                            verbose = TRUE) {
  data("PipekFilteredh_Coef", envir = environment())
  
  predAgev <- .calLinearClock(betaM, PipekFilteredh_Coef, "pipekFilteredh", 
                              minCoverage,verbose)
  # transformation
  predAgev <- .antiTrafo(predAgev)
  return(predAgev)
...
#' downloadOmniAgeRExample("Hannum_example")
#' loadOmniAgeRExample("Hannum_example")
#' pipekRetrainedhOut <- pipekRetrainedh(hannum_bmiq_m)
#'
...
pipekRetrainedh <- function(betaM,
                           minCoverage = 0.5, 
                           verbose = TRUE) {
  data("PipekRetrainedh_Coef", envir = environment())
  
  predAgev <- .calLinearClock(betaM, PipekRetrainedh_Coef, "pipekRetrainedh", 
                              minCoverage,verbose)
  # transformation
  predAgev <- .antiTrafo(predAgev)
  return(predAgev)

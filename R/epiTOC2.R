...
#' epitoc2Out <- epiTOC2(betaM = bmiq.m, age = df$Age)
#'
#' @export
#'
...
epiTOC2 <- function(betaM, age = NULL, minCoverage = 0.5, verbose = TRUE) {
  ...
  data('epiTOC2Model', envir = environment())
  if (!is.null(age) && length(age) != ncol(betaM)) {
    stop("[epiTOC2] Length of 'age' must match the number of samples in 'betaM'.")
  }
  
  # Obtain the list of epiTOC2 reference probes
  estParams <- epiTOC2Model 
  ...
  # Intrinsic Rate
  irS <- NULL; irS2 <- NULL; irT <- NULL; irT2 <- NULL

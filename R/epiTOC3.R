...
#' epitoc3Out <- epiTOC3(betaM = bmiq.m, age = df$Age)
#'
#' @export
#'
...
epiTOC3 <- function(betaM, age = NULL, minCoverage = 0.5, verbose = TRUE) {
  ...
  data('epiTOC3Model', envir = environment())
  if (!is.null(age) && length(age) != ncol(betaM)) {
    stop("Length of 'age' must match the number of samples in 'betaM'.")
  }
  
  # # Obtain the list of epiTOC3 reference probes
  estParams <- epiTOC3Model 

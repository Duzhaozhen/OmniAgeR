...
#'     `stochastic` + `ensemble` + `cross_species` + `celltype_specific`.
...
#'   as a single bundle.
#'   - `"celltype_specific"`: Calculates all cell-type-specific clocks 
...
#' (`McCartney_Trait`), disease risk prediction(e.g., `HepatoXuRisk`), 
...
#'
#' === Trait Prediction ===
#' * `McCartney_Trait`: (McCartney et al. 2018) The Epigenetic Surrogates for 
#' Complex Traits.
#'
...
#' * `Bohlin_GA`: Bohlin Gestational Age
#' * `EPIC_GA`: EPIC Gestational Age
#' * `Knight_GA`:  Knight Gestational Age
#' * `Lee_GA`:  A list (LeeControl, LeeRobust, LeeRefinedRobust). 
#' * `Mayne_GA`: Mayne Gestational Age
...
#' * `McCartney_Trait`: A list containing 10 named elements, one for each trait.
...
#' EpiAge.o <- epiMarker(betaM = bmiq.m, 
#'                       clockNames = "mitotic", 
#'                       chronAge = df$Age)
#'
...
#'
#' clock_names <- c("cellular_aging", "chronological", "biological", "causal", 
#' "stochastic")
#'
#' epiMarkerOut <- epiMarker(
#'   betaM = hannum_bmiq_m,
#'   clockNames = clock_names,
#'   chronAge = age,
#'   sexVec = sex,
#'   pcClockData = PCClocks_RData,
#'   systemsAgeData = SystemsAge_RData
#' )
#'
#' @export
#'
...
epiMarker <- function(betaM, 
  ...
  if (length(pcGroup) > 0 && !is.null(extraArgs$pcClockData)) {
    ...
    resultsList$PCClocks <- pcClocks(betaM, chronAge, sexVec, 
  ...
  if ("SystemsAge" %in% clocksToRun && !is.null(extraArgs$systemsAgeData)) {
    resultsList$SystemsAge <- systemsAge(betaM, extraArgs$systemsAgeData, 
                                         minCoverage, verbose)
  ...
  if ("EnsembleAge" %in% clocksToRun) {
    resultsList$EnsembleAge <- ensembleAge(betaM, extraArgs$ensembleVersion, minCoverage, verbose)
  ...
  if (length(panRequested) > 0) {
    if (is.null(extraArgs$speciesName) || is.null(extraArgs$anageData)) {
      ...
      resultsList <- .runPanMammalianLogic(resultsList, betaM, panRequested, 
  ...
  if (length(ctsGroup) > 0) {
    resultsList$CTSClocks <- ctsClocks(betaM, ctsClocks = ctsGroup,
  ...
  for (clockLabel in clocksToRun) {
    resultsList[[clockLabel]] <- .dispatchIndividualClock(clockLabel, betaM, 
                                                          chronAge, sexVec, 
                                                          minCoverage, verbose)
  }
...
.getClockCategories <- function() {
  ...
  list(
    ...
    ga = c("Bohlin_GA", "EPIC_GA", "Knight_GA", "Lee_GA", "Mayne_GA"),
    surrogateBiomarkers = c("CRP", "CHIP", "IL6", "EpiScores"),
    traitPred = c("McCartney_Trait"),
    diseaseRisk = c("SmokeIndex","HepatoXuRisk"),
...
.enforceDependencies <- function(clocksToRun) {
  if ("DNAmFitAge" %in% clocksToRun) {
    if (!any(c("GrimAge1", "GrimAge2") %in% clocksToRun)) {
      message("[EpiAge] Dependency: Adding 'GrimAge1' to run list as 
...
.filterByParameters <- function(clocks, chronAge, sexVec, verbose) {
  needsAge <- c("epiTOC2", "epiTOC3", "GrimAge1", "GrimAge2",
  ...
  if (is.null(chronAge)) {
    ...
    if (length(incompatible) > 0) {
      if (verbose) warning("[EpiAge] Missing 'chronAge'. Skipping: ", paste(incompatible, collapse = ", "))
  ...
  if (is.null(sexVec)) {
    ...
    if (length(incompatible) > 0) {
      if (verbose) warning("[EpiAge] Missing 'sexVec'. Skipping: ", paste(incompatible, collapse = ", "))
      removeList <- c(removeList, incompatible)
    }
  }
...
.dispatchIndividualClock <- function(clockLabel, betaM, chronAge, sexVec, minCoverage, verbose) {
  res <- switch(clockLabel,
                ...
                "McCartney_Trait" = mcCartneyTrait(betaM, minCoverage, verbose), 
                "SmokeIndex"  = compSmokeIndex(betaM, minCoverage, verbose),
                "HepatoXuRisk"    = hepatoXuRisk(betaM, minCoverage, verbose),
                
                {
                  warning(paste("[EpiMarker] Clock", clockLabel, "is not yet implemented in dispatcher."))
                  NULL
                }
  )
  return(res)

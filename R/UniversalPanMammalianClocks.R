...
universalPanMammalianClocks <- function(betaM, 
  ...
  data("PanMammalianClockCoef", envir = environment())
  ...
  anageSubset <- anageData[, c("SpeciesLatinName", "GestationTimeInYears", 
  ...
  for (k in seq_along(yNames)) {
    model_df <- PanMammalianClockCoef[[k]]
  ...
  # --- 5. Return Output ---
  final_results <- info[, setdiff(names(info), c("Y.pred1", "Y.pred2", "Y.pred3", "HighmaxAge"))]
